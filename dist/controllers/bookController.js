...
const checkBookExists = async (title, author) => {
    const result = await pool.query("SELECT * FROM books WHERE title = $1 AND author = $2", [title, author]);
    return result.rows.length > 0;
};
const checkBookExistsById = async (id) => {
...
const addBook = async (req, res) => {
    ...
    try {
        const result = await pool.query("INSERT INTO books (title, author, year, synopsis) VALUES ($1, $2, $3, $4) RETURNING *", [title, author, year, synopsis]);
